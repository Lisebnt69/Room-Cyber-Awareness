...
import { emails, logLines } from '../data/scenarioData'
...
function PhaseIntro({ onStart }) {
  ...
  const lines = [
    { t: 'ROOMCA — CENTRE DES OPÉRATIONS DE SÉCURITÉ', c: 'var(--red)' },
    { t: 'Initialisation du protocole de réponse aux incidents...', c: 'var(--text-muted)' },
    { t: 'ALERTE : Activité suspecte sur les identifiants détectée', c: '#f59e0b' },
    { t: 'Utilisateur ciblé : sophieb@acme-corp.com', c: '#f59e0b' },
    { t: 'Statut : COMPTE COMPROMIS — enquête requise', c: 'var(--red)' },
    { t: 'Mission : identifier le vecteur de phishing dans sa boîte mail', c: 'var(--text-secondary)' },
    { t: 'Limite de temps : 15:00 minutes', c: 'var(--text-secondary)' },
    { t: 'Bonne chance, analyste.', c: '#22c55e' },
  ]
  useEffect(() => {
    if (step < lines.length) {
      const t = setTimeout(() => setStep(s => s + 1), step === 0 ? 400 : 480)
    ...
    <div style={{ minHeight: '100vh', background: '#000', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px' }}>
      ...
      <div style={{ position: 'relative', zIndex: 2, width: '100%', maxWidth: '720px' }}>
        <div style={{ textAlign: 'center', marginBottom: '36px' }}>
          <div className="tag" style={{ display: 'inline-flex', marginBottom: '16px' }}><span className="status-dot red" /> INCIDENT DE SÉCURITÉ — ACTIF</div>
          <h1 style={{ fontFamily: 'var(--font-title)', fontSize: '28px' }}>OPÉRATION : <span style={{ color: 'var(--red)' }}>INBOX ZERO</span></h1>
        </div>
        ...
        {step >= lines.length && (
          <div style={{ display: 'flex', justifyContent: 'center', marginTop: '32px', animation: 'fadeInUp 0.5s ease' }}>
            <button className="btn-primary" onClick={onStart} style={{ fontSize: '15px', padding: '14px 44px' }}>Commencer l&apos;enquête →</button>
          </div>
        )}
      </div>
    </div>
    ...
    <div style={{ minHeight: '100vh', background: '#050505', display: 'flex', flexDirection: 'column', animation: glitch ? 'glitch 0.5s linear' : 'none' }}>
      <div style={{ background: '#080808', borderBottom: '1px solid var(--border-subtle)', padding: '12px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <div style={{ display: 'flex', gap: '32px', alignItems: 'center' }}>
          {[['SCÉNARIO', 'INBOX ZERO', 'var(--text-light)'], ['INDICES', `${found.length}/6`, 'var(--red)'], ['SCORE', score, '#22c55e'], ['TEMPS', timer.display, timer.seconds < 120 ? 'var(--red)' : 'var(--text-light)']].map(([lbl, val, col]) => (
        ...
        <div className="tag"><span className="status-dot red" /> En cours</div>
      </div>
      ...
      {showHint && (
        <div style={{ position: 'fixed', top: '80px', right: '24px', zIndex: 200, background: 'var(--bg-card)', border: '1px solid var(--red)', padding: '16px 20px', maxWidth: '320px', animation: 'fadeInUp 0.3s ease' }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--red)', letterSpacing: '0.15em', marginBottom: '8px' }}>🔍 INDICE DÉCOUVERT +{showHint.points} pts</div>
      ...
      <div style={{ display: 'flex', flex: 1 }}>
        <div style={{ width: '320px', flexShrink: 0, borderRight: '1px solid var(--border-subtle)', background: '#080808' }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border-subtle)', fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--text-muted)', letterSpacing: '0.15em' }}>BOÎTE DE RÉCEPTION — sophieb@acme-corp.com</div>
          {emails.map(email => (
              ...
              {classified[email.id] && (
                <div style={{ marginTop: '6px' }}>
                  <span style={{ fontSize: '10px', fontFamily: 'var(--mono)', padding: '2px 8px', border: '1px solid', borderColor: classified[email.id] === 'phishing' ? 'var(--red)' : '#22c55e', color: classified[email.id] === 'phishing' ? 'var(--red)' : '#22c55e', background: classified[email.id] === 'phishing' ? 'rgba(235,40,40,0.1)' : 'rgba(34,197,94,0.1)' }}>
                    {classified[email.id] === 'phishing' ? '⚠ PHISHING' : '✓ LÉGITIME'}
                  </span>
                </div>
              )}
            </div>
            ...
            <>
              <div style={{ padding: '24px 32px', borderBottom: '1px solid var(--border-subtle)' }}>
                ...
                <div style={{ display: 'flex', gap: '32px' }}>
                  <div>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--text-muted)', letterSpacing: '0.1em', marginBottom: '4px' }}>DE</div>
                    <div style={{ fontSize: '13px', cursor: 'pointer', padding: '4px 8px', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border-subtle)', transition: 'all 0.2s' }}
                      ...
                      title="Cliquez pour inspecter l'expéditeur"
                    >
                      {selected.from}{!selected.safe && <span style={{ marginLeft: '8px', color: 'var(--red)', fontSize: '11px', fontFamily: 'var(--mono)' }}>🔍</span>}
                    </div>
                  </div>
                  <div>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--text-muted)', letterSpacing: '0.1em', marginBottom: '4px' }}>HEURE</div>
              ...
              <div style={{ flex: 1, padding: '32px', overflowY: 'auto' }}>
                <div style={{ fontFamily: 'var(--mono)', fontSize: '13px', lineHeight: 2.2, color: 'var(--text-secondary)', whiteSpace: 'pre-line', maxWidth: '640px' }}>
                  {selected.body.split('\n').map((line, i) => {
                    if (selected.suspiciousLink && line.includes(selected.suspiciousLink)) {
                      return <span key={i}><span style={{ color: 'var(--red)', cursor: 'pointer', textDecoration: 'underline', fontWeight: 700 }} onClick={() => { const c = selected.clues?.find(cl => cl.type === 'link'); if (c) discoverClue(c) }} title={`Vrai URL : ${selected.suspiciousLink}`}>Cliquez ici pour vérifier votre compte</span>{`\n`}</span>
                    }
                    if (line.includes('24 heures') || line.includes('48 heures')) {
                ...
                <div style={{ marginTop: '32px', display: 'flex', gap: '12px' }}>
                  <button onClick={() => classify(selected.id, 'phishing')} style={{ padding: '10px 24px', fontFamily: 'var(--font-title)', fontSize: '12px', letterSpacing: '0.08em', background: classified[selected.id] === 'phishing' ? 'rgba(235,40,40,0.2)' : 'transparent', border: '1px solid var(--red)', color: 'var(--red)', cursor: 'pointer', transition: 'all 0.2s' }}>⚠ MARQUER PHISHING</button>
                  <button onClick={() => classify(selected.id, 'safe')} style={{ padding: '10px 24px', fontFamily: 'var(--font-title)', fontSize: '12px', letterSpacing: '0.08em', background: classified[selected.id] === 'safe' ? 'rgba(34,197,94,0.1)' : 'transparent', border: '1px solid #22c55e', color: '#22c55e', cursor: 'pointer', transition: 'all 0.2s' }}>✓ MARQUER LÉGITIME</button>
                </div>
              </div>
            </>
          ) : (
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '16px' }}>
              <div style={{ fontFamily: 'var(--mono)', fontSize: '14px', color: 'var(--text-muted)' }}>← Sélectionnez un e-mail à analyser</div>
              <div style={{ fontSize: '12px', color: '#333', fontFamily: 'var(--mono)' }}>Cliquez sur les éléments suspects pour découvrir des indices</div>
            </div>
          )}
        </div>
        ...
        <div style={{ width: '300px', flexShrink: 0, borderLeft: '1px solid var(--border-subtle)', background: '#060606', display: 'flex', flexDirection: 'column' }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border-subtle)', fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--text-muted)', letterSpacing: '0.15em' }}>JOURNAL SYSTÈME</div>
          ...
          <div style={{ borderTop: '1px solid var(--border-subtle)', padding: '16px' }}>
            <div style={{ fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--text-muted)', letterSpacing: '0.15em', marginBottom: '12px' }}>INDICES DÉCOUVERTS ({found.length}/6)</div>
            {found.length === 0 && <div style={{ fontSize: '11px', color: '#333', fontFamily: 'var(--mono)' }}>Aucun indice encore découvert</div>}
            {found.map(id => {
            ...
            {allClassified && (
              <button className="btn-primary" onClick={() => onComplete(score)} style={{ width: '100%', justifyContent: 'center', marginTop: '16px', fontSize: '12px', padding: '10px' }}>Terminer l’enquête →</button>
            )}
          </div>
        </div>
      </div>
    ...
    <div style={{ minHeight: '100vh', background: '#000', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px' }}>
      ...
      <div style={{ position: 'relative', zIndex: 2, width: '100%', maxWidth: '720px', animation: 'fadeInUp 0.6s ease' }}>
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-title)', fontSize: '32px', marginBottom: '8px' }}>{success ? 'Enquête réussie' : 'Mission échouée'}</h1>
          <div style={{ color: 'var(--text-muted)', fontSize: '14px' }}>{success ? 'Vous avez correctement identifié les menaces de phishing' : 'Certaines menaces sont passées inaperçues'}</div>
        </div>
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderTop: `2px solid ${success ? '#22c55e' : 'var(--red)'}`, padding: '36px', marginBottom: '24px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '32px', textAlign: 'center', marginBottom: '32px' }}>
            {[['SCORE FINAL', score, 'var(--red)', `/ ${maxScore} pts`], ['PRÉCISION', `${pct}%`, success ? '#22c55e' : '#f59e0b', ''], ['ROOMCOINS', `+${success ? 50 : 20}`, '#f59e0b', '']].map(([lbl, val, col, sub]) => (
              <div key={lbl}>
          ...
          <div style={{ fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--text-muted)', letterSpacing: '0.15em', marginBottom: '16px' }}>CE QUE VOUS AVEZ APPRIS</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {[
              ['🔍', 'Vérifiez toujours le domaine de l’expéditeur — les fautes de frappe sont un signal d’alarme majeur'],
              ['🔗', 'Survolez les liens avant de cliquer — l’URL réelle peut révéler le domaine malveillant'],
              ['⏰', 'La pression temporelle ("24h") est une tactique de manipulation — prenez le temps de vérifier'],
              ['💳', 'Les entreprises légitimes ne demandent jamais de paiement par e-mail non sollicité'],
            ].map(([icon, txt]) => (
              <div key={txt} style={{ display: 'flex', gap: '12px', fontSize: '13px', color: 'var(--text-secondary)', lineHeight: 1.6 }}><span>{icon}</span>{txt}</div>
            ))}
          </div>
        </div>
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
          {success && <button className="btn-primary" onClick={() => setShowCert(true)} style={{ padding: '12px 32px' }}>🏆 Voir mon certificat</button>}
          <button className="btn-secondary" onClick={onRetry} style={{ padding: '12px 32px' }}>↺ Rejouer</button>
          <button className="btn-secondary" onClick={onExit} style={{ padding: '12px 32px' }}>← Retour</button>
        </div>
      </div>
      {showCert && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px' }} onClick={() => setShowCert(false)}>
          <div style={{ background: '#0a0a0a', border: '1px solid var(--border)', maxWidth: '600px', width: '100%', padding: '48px', textAlign: 'center', position: 'relative', animation: 'fadeInUp 0.4s ease' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--text-muted)', letterSpacing: '0.3em', marginBottom: '24px' }}>ROOMCA — CERTIFICATION OFFICIELLE</div>
            <div style={{ fontFamily: 'var(--font-title)', fontSize: '13px', color: 'var(--text-muted)', marginBottom: '12px', letterSpacing: '0.15em' }}>CE CERTIFICAT EST DÉCERNÉ À</div>
            <div style={{ fontFamily: 'var(--font-title)', fontSize: '32px', color: 'var(--text-light)', marginBottom: '20px' }}>Analyste Certifié</div>
            <div style={{ width: '60px', height: '2px', background: 'var(--red)', margin: '0 auto 20px' }} />
            <div style={{ fontSize: '14px', color: 'var(--text-secondary)', marginBottom: '8px' }}>Pour avoir réussi le scénario</div>
            <div style={{ fontFamily: 'var(--font-title)', fontSize: '20px', color: 'var(--red)', marginBottom: '24px' }}>OPÉRATION : INBOX ZERO</div>
            <div style={{ fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--text-muted)' }}>Score : {score} pts — {new Date().toLocaleDateString('fr-FR')}</div>
