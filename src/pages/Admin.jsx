...
const employees = [
  ...
  { id: 3, name: 'Amélie Durand', dept: 'RH', score: 710, scenarios: 3, status: 'good', lastPlay: '03/04/2025' },
  { id: 4, name: 'Marc Lefebvre', dept: 'Commercial', score: 540, scenarios: 2, status: 'improve', lastPlay: '28/03/2025' },
  { id: 5, name: 'Julie Martin', dept: 'Direction', score: 880, scenarios: 4, status: 'excellent', lastPlay: '07/04/2025' },
  { id: 6, name: 'Pierre Rousseau', dept: 'Finance', score: 630, scenarios: 3, status: 'good', lastPlay: '04/04/2025' },
  { id: 7, name: 'Nadia Chouaib', dept: 'Commercial', score: 490, scenarios: 1, status: 'improve', lastPlay: '01/04/2025' },
...
function KpiCard({ label, value, sub, trend, accent }) {
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-subtle)', borderTop: accent ? '2px solid var(--red)' : '2px solid transparent', padding: '24px 28px', transition: 'border-color 0.2s' }}>
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#000', color: 'var(--text-light)' }}>
      ...
      <aside style={{ width: '240px', flexShrink: 0, background: '#080808', borderRight: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column', position: 'fixed', top: 0, left: 0, bottom: 0, zIndex: 50 }}>
        ...
        <div style={{ padding: '20px 24px', borderTop: '1px solid var(--border-subtle)' }}>
          ...
          <button onClick={() => { logout(); navigate('/login') }} className="btn-secondary" style={{ width: '100%', justifyContent: 'center', padding: '8px', fontSize: '12px' }}>
            {t('logout')}
          </button>
        </div>
      </aside>
      ...
      <main style={{ marginLeft: '240px', flex: 1, padding: '0' }}>
        <div style={{ padding: '20px 40px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#080808', position: 'sticky', top: 0, zIndex: 40 }}>
          <div>
            <h1 style={{ fontFamily: 'var(--font-title)', fontSize: '20px', fontWeight: 700 }}>{t('adminTitle')}</h1>
          ...
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
            ...
            <button className="btn-primary" style={{ padding: '8px 20px', fontSize: '12px' }}>{t('adminNewScenario')}</button>
          </div>
        </div>
            ...
            <div style={{ background: 'var(--bg-card)', padding: '28px' }}>
              ...
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={progressData}>
                  ...
                  <Area type="monotone" dataKey="participation" stroke="#eb2828" strokeWidth={2} fill="url(#gParticipation)" name={t('chartParticipation')} />
                  <Area type="monotone" dataKey="reussite" stroke="#545454" strokeWidth={2} fill="url(#gReussite)" name={t('chartReussite')} />
                </AreaChart>
              </ResponsiveContainer>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                  {[t('colEmployee'), t('colDept'), t('colScore'), t('colScenarios'), t('colStatus'), t('colLastPlay')].map(h => (
                    <th key={h} style={{ padding: '12px 28px', textAlign: 'left', fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--text-muted)', letterSpacing: '0.1em', fontWeight: 400 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.map((emp, i) => (
                  <tr key={emp.id} style={{ borderBottom: '1px solid var(--border-subtle)', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)', transition: 'background 0.15s', cursor: 'pointer' }}
