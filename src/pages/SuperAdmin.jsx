...
const companies = [
  { id: 1, name: 'ACME Corp', plan: 'Business', users: 161, active: 142, scenarios: 6, licenses: 200, expire: '31/12/2025', status: 'Actif' },
  { id: 2, name: 'BNP Finance', plan: 'Enterprise', users: 892, active: 814, scenarios: 12, licenses: 1000, expire: '30/06/2025', status: 'Actif' },
  { id: 3, name: 'Mairie de Lyon', plan: 'Starter', users: 24, active: 18, scenarios: 3, licenses: 25, expire: '15/05/2025', status: 'Expirant' },
  { id: 4, name: 'StartupTech SAS', plan: 'Starter', users: 12, active: 8, scenarios: 2, licenses: 25, expire: '01/09/2025', status: 'Actif' },
  { id: 5, name: 'Groupe Renault', plan: 'Enterprise', users: 2840, active: 2100, scenarios: 18, licenses: 3000, expire: '31/03/2026', status: 'Actif' },
]

const scenarios = [
  { id: 1, title: 'Opération : Inbox Zero', category: 'Phishing', difficulty: 'Intermédiaire', duration: '15 min', plays: 3241, score: 724, status: 'Publié' },
  { id: 2, title: 'Bureau Compromis', category: 'Ransomware', difficulty: 'Avancé', duration: '20 min', plays: 1892, score: 612, status: 'Publié' },
  { id: 3, title: 'Ingénierie Sociale', category: 'Social Eng.', difficulty: 'Débutant', duration: '10 min', plays: 4102, score: 831, status: 'Publié' },
  { id: 4, title: 'Fuite de Données', category: 'Insider', difficulty: 'Avancé', duration: '25 min', plays: 987, score: 568, status: 'Bêta' },
  { id: 5, title: 'WiFi Piégé', category: 'Réseau', difficulty: 'Intermédiaire', duration: '12 min', plays: 0, score: 0, status: 'Brouillon' },
]
...
const navItems = [
  ...
  { id: 'licenses', label: 'Licences', icon: '◈' },
  { id: 'system', label: 'Système', icon: '◎' },
]

const kpis = [
  { label: 'ENTREPRISES', value: '48', sub: '+3 ce mois', accent: true },
  { label: 'UTILISATEURS ACTIFS', value: '12 840', sub: '+284 ce mois' },
  { label: 'PARTIES JOUÉES', value: '89 412', sub: 'Total cumulé' },
  { label: 'MRR', value: '€24 800', sub: '+18% MoM' },
]

function statusBadge(s) {
  const map = {
    'Actif': { color: '#22c55e', bg: 'rgba(34,197,94,0.1)' },
    'Expirant': { color: '#f59e0b', bg: 'rgba(245,158,11,0.1)' },
    'Suspendu': { color: 'var(--red)', bg: 'rgba(235,40,40,0.1)' },
    'Publié': { color: '#22c55e', bg: 'rgba(34,197,94,0.1)' },
    'Bêta': { color: '#f59e0b', bg: 'rgba(245,158,11,0.1)' },
    'Brouillon': { color: 'var(--text-muted)', bg: 'rgba(84,84,84,0.1)' },
  }
  const style = map[s] || { color: 'var(--text-muted)', bg: 'transparent' }
  return (
    <span style={{
      padding: '3px 10px', fontSize: '11px', fontFamily: 'var(--mono)',
      color: style.color, border: `1px solid ${style.color}`, background: style.bg,
    }}>{s}</span>
  )
}
...
export default function SuperAdmin() {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#000', color: 'var(--text-light)' }}>
      {/* Sidebar */}
      <aside style={{
        width: '240px', flexShrink: 0, background: '#080808',
        ...
        <div style={{ padding: '28px 24px 20px', borderBottom: '1px solid var(--border-subtle)' }}>
          ...
          <div style={{ marginTop: '12px', padding: '8px 10px', background: 'rgba(235,40,40,0.12)', border: '1px solid rgba(235,40,40,0.3)' }}>
            ...
            <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginTop: '2px' }}>Accès total</div>
          </div>
        </div>
        ...
        <div style={{ padding: '20px 24px', borderTop: '1px solid var(--border-subtle)' }}>
          ...
          <button onClick={() => { logout(); navigate('/login') }}
            className="btn-secondary" style={{ width: '100%', justifyContent: 'center', padding: '8px', fontSize: '12px', marginTop: '8px' }}>
            Déconnexion
          </button>
        </div>
      </aside>
      ...
      <main style={{ marginLeft: '240px', flex: 1 }}>
        <div style={{
          ...
          <div>
            <h1 style={{ fontFamily: 'var(--font-title)', fontSize: '20px' }}>Console Super Admin</h1>
            <div style={{ fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--text-muted)', marginTop: '2px' }}>
              ROOMCA Platform v2.4.1 — 07/04/2025
            </div>
          </div>
          <div className="tag"><span className="status-dot red" /> Environnement Production</div>
        </div>
        ...
        <div style={{ padding: '40px' }}>
          ...
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-subtle)', marginBottom: '32px' }}>
            <div style={{ padding: '20px 28px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button className="btn-primary" style={{ padding: '8px 20px', fontSize: '12px' }}>+ Ajouter</button>
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                  {['Entreprise', 'Plan', 'Utilisateurs', 'Scénarios', 'Licences', 'Expiration', 'Statut', ''].map(h => (
                    <th key={h} style={{ padding: '12px 20px', textAlign: 'left', fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--text-muted)', letterSpacing: '0.1em', fontWeight: 400 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {companies.map((c, i) => (
                  ...
                  >
                    ...
                    <td style={{ padding: '14px 20px' }}>
                      <span style={{ fontFamily: 'var(--mono)', fontSize: '11px', color: c.plan === 'Enterprise' ? 'var(--red)' : 'var(--text-muted)', border: '1px solid', borderColor: c.plan === 'Enterprise' ? 'var(--red)' : 'var(--border)', padding: '2px 8px' }}>{c.plan}</span>
                    </td>
                    ...
                    <td style={{ padding: '14px 20px', fontFamily: 'var(--mono)', fontSize: '11px', color: c.status === 'Expirant' ? '#f59e0b' : 'var(--text-muted)' }}>{c.expire}</td>
          ...
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-subtle)' }}>
            <div style={{ padding: '20px 28px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button className="btn-primary" style={{ padding: '8px 20px', fontSize: '12px' }}>+ Créer scénario</button>
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                  {['Titre', 'Catégorie', 'Niveau', 'Durée', 'Parties', 'Score moy.', 'Statut', ''].map(h => (
                    <th key={h} style={{ padding: '12px 20px', textAlign: 'left', fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--text-muted)', letterSpacing: '0.1em', fontWeight: 400 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {scenarios.map((s, i) => (
                  <tr key={s.id} style={{
                    ...
                    onMouseLeave={e => e.currentTarget.style.background = i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)'}
                  >
                    <td style={{ padding: '14px 20px', fontSize: '13px', fontWeight: 500 }}>{s.title}</td>
                    <td style={{ padding: '14px 20px' }}><span className="tag" style={{ fontSize: '10px', padding: '2px 8px' }}>{s.category}</span></td>
                    <td style={{ padding: '14px 20px', fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--text-muted)' }}>{s.difficulty}</td>
